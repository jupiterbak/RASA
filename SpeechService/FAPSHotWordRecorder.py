...
from SpeechService.FAPSListener import FAPSListener
...
def _default_library_path():
    ...
    if system == 'Darwin':
        return os.path.join(os.path.dirname(__file__), 'lib/porcupine/library/mac/%s/libpv_porcupine.dylib' % machine)
    elif system == 'Linux':
        if machine == 'x86_64' or machine == 'i386':
            return os.path.join(os.path.dirname(__file__), 'lib/porcupine/library/linux/%s/libpv_porcupine.so' % machine)
        else:
        ...
        if platform.architecture()[0] == '32bit':
            return os.path.join(os.path.dirname(__file__),
                                'lib\\porcupine\\library\\windows\\i686\\libpv_porcupine.dll')
        else:
            return os.path.join(os.path.dirname(__file__),
                                'lib\\porcupine\\library\\windows\\amd64\\libpv_porcupine.dll')
...
class FAPSHotWordRecorder(multiprocessing.Process):
    ...
    def run(self):
        model_file_path = "./ressources/porcupine_model/porcupine_params.pv"
